...
const UserOrders = () => {
  ...
  const {setOpenTrackOrder} = useOpenTrackOrder()
            ...
            <Card key={index} className="my-4 shadow-md">
              <CardContent className="p-4">
                <Box className="flex justify-end">
                  <MyButton onClick={setOpenTrackOrder} $color='true'>
                    Track Order
                  </MyButton>
                </Box>
              ...
              {item?.orderItems?.map((product, index) => (
                <CardContent key={index} className="p-4">
                  <Box className="flex justify-between items-center">
                    ...
                    <Box >
                        <span>Status</span>
                        <Typography variant="body2"  className="text-orange-600 !text-lg !font-bold">{item?.orderStatus}</Typography>
                      </Box>
                    </Box>
              ...
              <Box className="p-4 border-t border-gray-200 flex justify-between items-center">
                <MyButton $color="true" className="text-gray-500">
                  Cancel Order
                </MyButton>
